...
describe('handlePullRequest', () => {
  ...
  it('includes attribution footer on summary comment', async () => {
    const ai = mockAI('This PR adds a new feature.');
    const github = mockGitHub();
    const payload = { pull_request: { number: 10, title: 'Add feature', body: 'Description' } };

    await handlePullRequest(payload, ai, github as never, baseConfig);

    const commentCall = github.addComment.mock.calls[0];
    expect(commentCall[1]).toContain('Powered by [RepoKeeper]');
    // Must NOT have the old hardcoded footer
    expect(commentCall[1]).not.toContain('Generated by [RepoKeeper]');
  });

  it('omits attribution footer when disabled', async () => {
    const ai = mockAI('This PR adds a new feature.');
    const github = mockGitHub();
    const config = { ...baseConfig, attribution: { enabled: false } };
    const payload = { pull_request: { number: 11, title: 'Add feature', body: 'Description' } };
...
describe('handlePullRequestMerged', () => {
  ...
  it('includes attribution footer on release notes', async () => {
    const ai = mockAI('## v1.0\n- Added new feature');
    const github = mockGitHub();
    const payload = { pull_request: { number: 30, title: 'Add feature', body: 'New feature', merged: true } };
